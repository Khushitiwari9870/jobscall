...
export default function SentPage() {
  ...
  const fetchSentEmails = useCallback(async () => {
    try {
      setIsLoading(true);
      // Filter for sent emails by type
      const response = await fetch(
        `/api/v1/emails/logs/?page=${currentPage}&page_size=${itemsPerPage}&search=${searchQuery}&type=sent`
      );
  ...
  const getDeliveryStatus = (email: Email) => {
    // This is a simplified example - you would check the actual status from your API
    if (email.sentAt) {
      return { text: 'Delivered', icon: <FiCheck className="h-3.5 w-3.5 text-green-500" /> };
    }
    return { text: 'Sending...', icon: <FiClock className="h-3.5 w-3.5 text-yellow-500" /> };
  };
      ...
      <div className="divide-y divide-gray-200">
        {emails.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            ...
            <h3 className="mt-2 text-sm font-medium">No sent emails</h3>
